...
template <typename T,
    ...
    int PIPE_MAX = 3,
    bool FUSE_RES = false>
    __device__ __forceinline__ void linear_gemm_tl_1_6144_1024_1(const int bx, const int by, const int bz,
...
template <typename T,
    ...
    int PIPE_MAX = 3,
    bool FUSE_RES = false>
    __device__ __forceinline__ void linear_gemm_tl_1_6144_1024_2(const int bx, const int by, const int bz,
      uint64_t* mbarrier_mem, const CUtensorMap *A_desc, const CUtensorMap *B_desc, const void* __restrict__ residual_ptr, const CUtensorMap *C_desc, 
                                                int num_active_tokens,
                                                bool residual) {
  static_assert(THREAD_NUM==256);
...
extern "C" int create_linear_gemm_tl_1_6144_1024(bfloat16_t* __restrict__ A, bfloat16_t* __restrict__ B, bfloat16_t* __restrict__ C, CUtensorMap* out_A_desc, CUtensorMap* out_B_desc, CUtensorMap* out_C_desc, bool to_device) {
	...
	//	linear_kernel<<<dim3(96, 1, 1), dim3(256, 1, 1), 61440, stream>>>(A_desc, B_desc, C_desc);
