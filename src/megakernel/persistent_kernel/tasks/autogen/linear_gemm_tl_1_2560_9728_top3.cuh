...
template <typename T,
  ...
  for (int i_7 = 0; i_7 < 16; ++i_7) {
    ...
    *(uint1*)(((bfloat16_t*)buf_dyn_shmem) + ((((((((int)threadIdx.x) >> 5) * 1024) + ((i_7 & 1) * 512)) + (((((int)threadIdx.x) & 31) >> 2) * 64)) + ((i_7 >> 1) * 8)) + ((((int)threadIdx.x) & 3) * 2))) = __1;
  }
  __syncthreads();
  #pragma unroll
  for (int i_8 = 0; i_8 < 4; ++i_8) {
    if (((i_8 * 16) + (((int)threadIdx.x) >> 3)) < 1) {
      *(uint4*)(C + ((((i_8 * 40960) + ((((int)threadIdx.x) >> 3) * 2560)) + (((int)bx) * 64)) + ((((int)threadIdx.x) & 7) * 8))) = *(uint4*)(((bfloat16_t*)buf_dyn_shmem) + ((i_8 * 1024) + (((int)threadIdx.x) * 8)));
    }
  }
}
