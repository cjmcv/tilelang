...
template <typename T,
        ...
        int M,
        int N>
__device__ __forceinline__ void rms_norm_kernel_128_2560(const int bx, const int by, const int bz,
  ...
  static_assert(M==128); static_assert(N==2560);
  ...
  #pragma unroll
  for (int i = 0; i < 5; ++i) {
    *(uint2*)(((bfloat16_t*)buf_dyn_shmem) + ((i * 512) + (((int)threadIdx.x) * 4))) = *(uint2*)(A + (((((int)bx) * 2560) + (i * 512)) + (((int)threadIdx.x) * 4)));
  }
  #pragma unroll
  ...
  for (int i_6 = 0; i_6 < 5; ++i_6) {
    ...
    *(uint2*)(C + (((((int)bx) * 2560) + (i_6 * 512)) + (((int)threadIdx.x) * 4))) = __3;
  }
}


} // kernel
// Strategy: rms_norm_tl_128_2560
// selected_hparams: [1, 1, 128].
// smem: 10240 bytes.
// use_cooperative_groups: 0.
// grid_dim=(128, 1, 1), tile_dim=(1, 1, 1)
// block_dim=(128, 1, 1).