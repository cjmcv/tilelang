...
template <typename T,
          ...
          int I_STRIDE,
          int O_STRIDE>
__device__ __forceinline__ void silu_mul_kernel_128_9728(const int bx, const int by, const int bz,
                                                   void const *input_ptr,
                                                   void *output_ptr,
                                                   int num_active_tokens) {
  static_assert(THREAD_NUM==128);
  static_assert(TILE_DIM_X==64); static_assert(TILE_DIM_Y==32); static_assert(TILE_DIM_Z==1);
  static_assert(M==128); static_assert(N==9728);
  ...
  #pragma unroll
  for (int i = 0; i < 2; ++i) {
    *(uint4*)(((bfloat16_t*)buf_dyn_shmem) + ((i * 1024) + (((int)threadIdx.x) * 8))) = *(uint4*)(A + (((((((int)by) * 622592) + (i * 311296)) + ((((int)threadIdx.x) >> 3) * 19456)) + (((int)bx) * 64)) + ((((int)threadIdx.x) & 7) * 8)));
  }
  #pragma unroll
  for (int i_1 = 0; i_1 < 2; ++i_1) {
    *(uint4*)(((bfloat16_t*)buf_dyn_shmem) + (((i_1 * 1024) + (((int)threadIdx.x) * 8)) + 2048)) = *(uint4*)(A + ((((((((int)by) * 622592) + (i_1 * 311296)) + ((((int)threadIdx.x) >> 3) * 19456)) + (((int)bx) * 64)) + ((((int)threadIdx.x) & 7) * 8)) + 9728));
  }
  __syncthreads();
  #pragma unroll
  ...
  for (int i_3 = 0; i_3 < 2; ++i_3) {
    *(uint4*)(C + (((((((int)by) * 311296) + (i_3 * 155648)) + ((((int)threadIdx.x) >> 3) * 9728)) + (((int)bx) * 64)) + ((((int)threadIdx.x) & 7) * 8))) = *(uint4*)(((bfloat16_t*)buf_dyn_shmem) + (((i_3 * 1024) + (((int)threadIdx.x) * 8)) + 4096));
  }
}


} // kernel
// Strategy: silu_mul_tl_128_9728
// selected_hparams: [32, 64, 128].
// smem: 12288 bytes.
// use_cooperative_groups: 0.
// grid_dim=(152, 4, 1), tile_dim=(64, 32, 1)
// block_dim=(128, 1, 1).