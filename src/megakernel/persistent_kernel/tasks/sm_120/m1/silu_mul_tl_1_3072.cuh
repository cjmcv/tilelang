...
__device__ __forceinline__ void silu_mul_kernel_1_3072(const int bx, const int by, const int bz,
  ...
  *(uint4*)(((bfloat16_t*)buf_dyn_shmem) + ((((int)threadIdx.x) * 8) + 2048)) = condval_1;
  __syncthreads();
  #pragma unroll
  for (int i = 0; i < 2; ++i) {
    float4 __1;
    uint2 v_ = *(uint2*)(((bfloat16_t*)buf_dyn_shmem) + ((i * 1024) + (((int)threadIdx.x) * 4)));
    ...
    float4 __7;
      float4 __8;
        ...
        __8.w = (xi.w*sig.w);
      float4 __9;
      uint2 v__3 = *(uint2*)(((bfloat16_t*)buf_dyn_shmem) + (((i * 1024) + (((int)threadIdx.x) * 4)) + 2048));
    ...
    *(uint2*)(((bfloat16_t*)buf_dyn_shmem) + (((i * 1024) + (((int)threadIdx.x) * 4)) + 4096)) = __6;
  }
  __syncthreads();
  if (((int)threadIdx.x) < 8) {
    *(uint4*)(C + ((((int)bx) * 64) + (((int)threadIdx.x) * 8))) = *(uint4*)(((bfloat16_t*)buf_dyn_shmem) + ((((int)threadIdx.x) * 8) + 4096));
  }
}
