...
template <typename T,
  ...
  __shared__ uint64_t mbarrier_mem[3];
  auto mbarrier = reinterpret_cast<Barrier*>(mbarrier_mem);
  if (tl::tl_shuffle_elect<0>()) {
    tl::prefetch_tma_descriptor(*A_desc);
    tl::prefetch_tma_descriptor(*B_desc);
    tl::prefetch_tma_descriptor(*R_desc);
  ...
  if (128 <= ((int)threadIdx.x)) {
    ...
    if (tl::tl_shuffle_elect<128>()) {
      mbarrier[2].arrive_and_expect_tx(2048);
      tl::fence_proxy_async();
      tl::tma_load(*R_desc, mbarrier[2], (&(((bfloat16_t*)buf_dyn_shmem)[10240])), (((int)bx) * 64), 0);
    }
  } else {
    ...
    for (int i_1 = 0; i_1 < 4; ++i_1) {
      float2 c_val = *(float2*)(C_local + (i_1 * 2));
      float2 __1;
      uint1 v_ = *(uint1*)(((bfloat16_t*)buf_dyn_shmem) + (((((((i_1 & 1) * 512) + (((((int)threadIdx.x) & 31) >> 2) * 64)) + ((((int)threadIdx.x) >> 5) * 16)) + ((i_1 >> 1) * 8)) + ((((int)threadIdx.x) & 3) * 2)) + 10240));
      ...
      *(uint1*)(((bfloat16_t*)buf_dyn_shmem) + (((((((i_1 & 1) * 512) + (((((int)threadIdx.x) & 31) >> 2) * 64)) + (((((((((int)threadIdx.x) >> 5) * 16) + ((i_1 >> 1) * 8)) >> 5) + ((((int)threadIdx.x) & 31) >> 4)) & 1) * 32)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 15) >> 3)) & 1) * 16)) + (((((((int)threadIdx.x) & 7) >> 2) + (i_1 >> 1)) & 1) * 8)) + ((((int)threadIdx.x) & 3) * 2))) = __2;
    }
    tl::fence_proxy_async();
    tl::__sync_thread_partial<3, 128>();
    if (tl::tl_shuffle_elect<128>()) {
...
extern "C" int create_linear_gemm_add_tl_1_1024_2048(bfloat16_t* __restrict__ A, bfloat16_t* __restrict__ B, bfloat16_t* __restrict__ R, bfloat16_t* __restrict__ C, CUtensorMap* out_A_desc, CUtensorMap* out_B_desc, CUtensorMap* out_C_desc, CUtensorMap* out_R_desc, bool to_device) {

	CUtensorMap A_desc;
	CUtensorMapDataType A_desc_type= (CUtensorMapDataType)9;
	cuuint32_t A_desc_tensorRank= 2;
	void *A_desc_globalAddress= A;
	cuuint64_t A_desc_globalDim[2]= {2048,1};
	cuuint64_t A_desc_globalStride[2]= {2,4096};
	...
	cuuint64_t B_desc_globalDim[2]= {2048,1024};
	cuuint64_t B_desc_globalStride[2]= {2,4096};
	...
	CUtensorMap R_desc;
	CUtensorMapDataType R_desc_type= (CUtensorMapDataType)9;
	cuuint32_t R_desc_tensorRank= 2;
	void *R_desc_globalAddress= R;
	cuuint64_t R_desc_globalDim[2]= {1024,1};
	cuuint64_t R_desc_globalStride[2]= {2,2048};
	cuuint32_t R_desc_boxDim[2]= {64,16};
	cuuint32_t R_desc_elementStrides[2]= {1,1};
	CUtensorMapInterleave R_desc_interleave= (CUtensorMapInterleave)0;
	CUtensorMapSwizzle R_desc_swizzle= (CUtensorMapSwizzle)0;
	CUtensorMapL2promotion R_desc_l2Promotion= (CUtensorMapL2promotion)2;
	CUtensorMapFloatOOBfill R_desc_oobFill= (CUtensorMapFloatOOBfill)0;

	CUresult R_desc_result = CUTLASS_CUDA_DRIVER_WRAPPER_CALL(cuTensorMapEncodeTiled)(
    &R_desc, R_desc_type, R_desc_tensorRank, R_desc_globalAddress, R_desc_globalDim, R_desc_globalStride + 1, R_desc_boxDim, R_desc_elementStrides, R_desc_interleave, R_desc_swizzle, R_desc_l2Promotion, R_desc_oobFill);

	if (R_desc_result != CUDA_SUCCESS) {
		printf("Error: Failed to initialize the TMA descriptor A_desc");
		return -1;
	}
	if (to_device) {
		...
		cudaMemcpy(out_R_desc, &R_desc, sizeof(CUtensorMap), cudaMemcpyHostToDevice);
	} else {
		*out_A_desc = A_desc;
		*out_B_desc = B_desc;
		*out_C_desc = C_desc;
	...
	//	linear_kernel<<<dim3(16, 1, 1), dim3(256, 1, 1), 22528, stream>>>(A_desc, B_desc, C_desc, R_desc);
