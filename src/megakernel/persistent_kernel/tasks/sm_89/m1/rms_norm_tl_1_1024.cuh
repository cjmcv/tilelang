...
__device__ __forceinline__ void rms_norm_kernel_1_1024(const int bx, const int by, const int bz,
  ...
  float A_pow_local[4];
  float A_powsum[1];
  *(uint2*)(((bfloat16_t*)buf_dyn_shmem) + (((int)threadIdx.x) * 4)) = *(uint2*)(A + (((int)threadIdx.x) * 4));
  *(uint2*)(((bfloat16_t*)buf_dyn_shmem) + ((((int)threadIdx.x) * 4) + 1024)) = *(uint2*)(B + (((int)threadIdx.x) * 4));
  float4 __1;
