...
def get_color_map(prefix: Literal["TASK", "EVENT"]) -> dict[int, tuple[str, str]]:
    # Use MIRAGE_HOME
    filename = os.path.join(os.environ["MIRAGE_HOME"], "src/mirage/persistent_kernel/runtime_header.h")
...
def check_supported_data_types() -> None:
    filename = os.path.join(os.environ["MIRAGE_HOME"], "src/mirage/type.h")
    with open(filename, "r") as f:
        lines = f.readlines()
