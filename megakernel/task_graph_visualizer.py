...
def get_color_map(prefix: Literal["TASK", "EVENT"]) -> dict[int, tuple[str, str]]:
    # Use MEGAKERNEL_HOME
    filename = os.path.join(os.environ["MEGAKERNEL_HOME"], "src/megakernel/persistent_kernel/runtime_header.h")
...
def check_supported_data_types() -> None:
    filename = os.path.join(os.environ["MEGAKERNEL_HOME"], "src/megakernel/type.h")
    with open(filename, "r") as f:
        lines = f.readlines()
