...
class JITKernel(Generic[_P, _T]):
    ...
    def get_dispatch_source(self, kernel_only: bool = True) -> str:
        ...
        if isinstance(self.prim_func, PrimFunc):
            ir_module = tvm.IRModule({self.prim_func.attrs["global_symbol"]: self.prim_func})
        else:
            ir_module = self.prim_func
        self.wrapper = TLWrapper(self.target)
        self.wrapper.assign_optimized_module(ir_module)
        self.wrapper.assign_pass_configs(self.pass_configs)
        self.wrapper.assign_host_module(my_artifact.host_mod)
        self.wrapper.assign_device_module(my_artifact.device_mod)
        return self.wrapper.wrap(my_artifact.kernel_source)
